...
// `MatchLiquidationOrder` entry point through `matchLiquidationLoop`
...
//   - NoRiskCheck       = true (victim is being closed; maker on the
//     public book was vetted at order placement)
//
...
func TestMatchLiquidation_PerpFillCarriesLiquidationFields(t *testing.T) {
	...
	require.True(t, got.NoRiskCheck,
		"liquidation fill must skip both maker and taker post-state risk checks")
}
