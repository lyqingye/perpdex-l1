...
## 8. 2026-05-05 增量：清算机制对齐 Lighter

针对 [Lighter Liquidations & LLP](https://docs.lighter.xyz/trading/liquidations-and-llp-insurance-fund) 文档，对清算瀑布做一次性对齐，覆盖原审查报告之外的 8 个偏差。详情：

| 偏差 | 旧实现 | 新实现 |
|---|---|---|
| 零价公式 | `entry_quote / position`（入场均价） | `mark * (1 ± M_i * TAV / MMR)`，cross 用 `ComputeRiskInfo`，isolated 用 `ComputeIsolatedRisk` |
| 预清算下单门禁 | 无任何限制；EndBlocker 只清 PRE flag | `x/matching` `CreateOrder/ModifyOrder` 入口检查 cross + isolated 健康度：PRE → 仅 reduce-only；PARTIAL/FULL/BANKRUPTCY → 全拒（`ErrAccountUnderLiquidation`） |
| 部分清算前置撤单 | `MsgLiquidate` 直接 fill，victim 残留挂单 | `Liquidate` 调用 `MatchingKeeper.CancelAllOpenOrdersForAccount` 后再下零价 IoC |
| 清算费 | victim 按 notional × `LiquidationFee` 进 Treasury | `Fill.{ZeroPrice, LiquidationFeeBps, LiquidationFeeRecipient}`：fee = min(improvement × bps / FeeTick, notional/100)，进 LLP；零价成交（keeper 路径）改善量为 0 → fee=0 |
| LLP 接盘排序 | `tryIFAbsorb` 整仓塞 IF | `tryLLPAbsorb` 按 victim uPnL 升序排队，`SimulateRiskAfterTakeover` 校验 LLP `postTAV >= postIMR`，否则跳过该仓走 ADL |
| ADL 触发条件 | 仅 `HealthBankruptcy` | `HealthFullLiquidation` + `HealthBankruptcy` 都先尝试 LLP，LLP 拒绝的剩余走 `autoADL` |
| ADL 排名 | uPnL × MarginTick / collateral | `Score = leverage × uPnL_ratio` 降序，账户 idx 升序 tiebreak |
| ADL 零价对齐 | 用 victim 的零价单边成交 | `victimZP` 与 `candZP` 必须重叠（victim long 要求 `victimZP <= candZP`，short 要求 `victimZP >= candZP`），成交价取 `(victimZP + candZP) / 2` |
| Isolated 健康聚合 | 把 isolated `Allocated + uPnL` 加进 cross TAV，但不累加 IM/MM/CM | cross 只聚合 cross；`GetIsolatedHealthStatus` 提供单仓 isolated 健康；`IsValidRiskChange` 同时校验 cross 和每个 isolated 仓位 |

`IsValidRiskChange` 在 PRE 阶段额外要求：`post.MMR <= pre.MMR`（同 block mark 不变 → 等价于「不增加任意仓位 size」）+ `TAV/MMR` 比值不下降。`SnapshotPreRisk` 同时缓存 cross 和每个 isolated 仓位的 pre-state。

新增/重写测试：

- `x/risk`：mark-based 零价（long/short/cross/isolated）、isolated 不污染 cross、isolated 独立健康判定、PRE 下 MMR 增长被拒、PRE 下减仓被允许。
- `x/liquidation`：清算前置 cancel-all、清算费入 LLP、FULL 走 LLP→ADL、LLP 不破 IMR 时回退 ADL、BANKRUPTCY+LLP 不可吸收回退 ADL、ADL 零价对齐、ADL leverage+uPnL 排名、PRE clears flag。
- `x/matching`：8 种 cross/iso × reduce-only/普通的 PRE 门禁矩阵。
- `x/trade`：improvement-based fee 入 LLP；zero-price 成交 fee=0；treasury 不收取清算费。

外部 keeper bot 行为变化：`GetPositionZeroPrice` 返回值含义变化（不再是入场均价），需重读规格。「问题 2（Liquidate 数量未封顶）」已在 PR 之前合入；本次保留同样的 cap 检查并扩展到 `Deleverage` 路径以兼容 LLP 大额接盘。
